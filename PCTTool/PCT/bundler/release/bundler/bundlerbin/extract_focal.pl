...
%ccd_widths =
    ("NIKON E4500"                      => 7.176,  # 1/1.8"
     ...
     "NIKON CORPORATION NIKON D300"     => 23.6,
...
#    }
	...
	$line = sprintf("%s.jpg 0 %0.5f", $basename, 2133.0);
#	$line = sprintf("%s.jpg", $basename);
    }

    `echo $line >> $OUT_DIR/list_focal.txt`;
    $num_output_images++;
}
